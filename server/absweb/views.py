...
from cups import HTTP_STATUS_SERVER_ERROR, HTTP_STATUS_BAD_REQUEST
    ...
    try:
        if request.method == 'POST':
            ...
            try:
                ...
                return HttpResponse("Unable to parse data.", status=HTTP_STATUS_BAD_REQUEST)
            ...
            while str(key) in dict_data:
                if (float(dict_data[str(key)]['0']) <= 1) and (float(dict_data[str(key)]['1']) >= 0) and \
                    ...
                    return HttpResponse("Wrong type of data.", status=HTTP_STATUS_BAD_REQUEST)
            ...
            return HttpResponse("Wrong request method: POST required.", status=HTTP_STATUS_BAD_REQUEST)
    except Exception:
        return HttpResponse("Server error", status=HTTP_STATUS_SERVER_ERROR)
