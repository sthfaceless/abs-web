...
import requestСonsistencyChecking from "utils/httputils";
...
export default function KpPanel(props) {
    ...
    const reconcile = () => {
        ...
        requestСonsistencyChecking(data,
            (result) => {
            ...
            {[...Array((1 << baseNumber)).keys()].map(index => {
                ...
                return (isVisibleInput(index, props.kpType, props.kpTypes) &&
                    <InputEstimations expression={expression} key={index} estimationType={estimationType}
                                      ...
                                      firstValue={(fields[index] && (typeof fields[index].value[0] !== 'undefined')) ? fields[index].value[0] : ''}
                                      secondValue={(fields[index] && (typeof fields[index].value[1] !== 'undefined')) ? fields[index].value[1] : ''}
